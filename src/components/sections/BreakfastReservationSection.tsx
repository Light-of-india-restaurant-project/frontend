...
import { api, BreakfastReservationData, OpenDate } from "@/lib/api";
import { useToast } from "@/hooks/use-toast";

interface BreakfastReservationSectionProps {
  sectionId?: string;
}

const BreakfastReservationSection = ({ sectionId = "breakfast-reservation" }: BreakfastReservationSectionProps) => {
  ...
  useEffect(() => {
    const fetchOpenDates = async () => {
      ...
      try {
        const response = await api.getBreakfastReservationOpenDates();
        setOpenDates(response.data);
      } catch (err) {
        console.error("Failed to fetch breakfast open dates:", err);
        toast({
          variant: "destructive",
          title: language === "nl" ? "Kon datums niet laden" : "Could not load dates",
          description: language === "nl" ? "Probeer het opnieuw." : "Please try again.",
        });
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const reservationData: BreakfastReservationData = {
      ...
      await api.createBreakfastReservation(reservationData);

      setIsSuccess(true);
      toast({
        title: language === "nl" ? "Ontbijtreservering verzonden!" : "Breakfast reservation sent!",
        description:
          language === "nl"
            ? "We nemen snel contact met u op om uw ontbijtreservering te bevestigen."
            : "We will contact you soon to confirm your breakfast reservation.",
        duration: 10000,
      });
      ...
      toast({
        variant: "destructive",
        title: language === "nl" ? "Ontbijtreservering mislukt" : "Breakfast reservation failed",
        description: language === "nl" ? "Probeer het opnieuw of bel ons direct." : "Please try again or call us directly.",
        duration: 5000,
      });
    } finally {
      setIsSubmitting(false);
  ...
  return (
    <section id={sectionId} className="py-24 bg-brown text-cream">
      <div className="container mx-auto px-6">
        <SectionHeading title={t("breakfastReservation.title")} subtitle={t("breakfastReservation.subtitle")} />
        ...
        <div className="max-w-lg mx-auto">
          {isSuccess ? (
            <div className="text-center py-12 animate-fade-in">
              ...
              <h3 className="font-display text-2xl mb-4">{t("breakfastReservation.successTitle")}</h3>
              <p className="font-serif text-cream/80 mb-4">{t("breakfastReservation.successDescription")}</p>
            </div>
          ) : todayClosureReason ? (
            <div className="text-center py-10 bg-amber-500/15 border border-amber-400/40 rounded-lg">
              <h3 className="font-display text-2xl mb-3 text-amber-100">{t("closure.closedTodayTitle")}</h3>
              <p className="font-serif text-amber-100/90 max-w-md mx-auto">{todayClosureReason}</p>
              <p className="font-serif text-cream/80 text-sm mt-4">{t("closure.reservationsDisabled")}</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                {isLoadingDates ? (
                  <div className="flex items-center justify-center py-3 bg-cream/10 border border-cream/20">
                    ...
                    <span className="font-serif text-cream/60">{language === "nl" ? "Datums laden..." : "Loading dates..."}</span>
                  </div>
                  ...
                  >
                    ...
                    {openDates.map((dateObj) => (
                      ...
                      >
                        {formatDateOption(dateObj.date, dateObj.dayName)}
                        {!dateObj.isOpen ? (language === "nl" ? " - Gesloten" : " - Closed") : ""}
                      </option>
                    ))}
                  </select>
                )}
              ...
              <button
                type="submit"
                disabled={isSubmitting || !formData.reservationDate || !formData.name || !formData.email || !formData.contactNumber || !!selectedDateClosureReason}
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    {t("breakfastReservation.submitting")}
                  </>
                ) : (
                  t("breakfastReservation.submit")
                )}
              </button>

              <p className="text-center text-cream/80 text-lg font-serif mt-4">
                {language === "nl" ? "Voor grotere groepen of speciale wensen, bel ons op " : "For larger parties or special requests, please call us at "}
...
export default BreakfastReservationSection;
